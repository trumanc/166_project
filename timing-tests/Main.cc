...
int main() {
  ...
  std::cout << "  VebTree:           " << (checkCorrectness<VebTree>(kTreeSize, kNumLookups) ? "pass" : "fail") << std::endl;
  ...
  std::cout << "  VebTree:           " << timeSequential<VebTree>(kTreeSize) << " ms" << std::endl;
  ...
  std::cout << "  VebTree:           " << timeReverseSequential<VebTree>(kTreeSize) << " ms" << std::endl;
  ...
  std::cout << "  VebTree:           " << timeWorkingSets<VebTree>(kNumWorkingSets, kNumWorkingSets, kNumLookups) << " ms" << std::endl;
  ...
  std::cout << "  VebTree:           " << timeDistribution<VebTree>(uniform, kNumLookups) << " ms" << std::endl;
  ...
  for (double z: {0.5, 0.75, 1.0, 1.2, 1.3}) {
    ...
    std::cout << "  VebTree:           " << timeDistribution<VebTree>(distribution_z, kNumLookups) << " ms" << std::endl;
